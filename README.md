...
- **표본 한계**: 학습 데이터가 59개로 매우 제한적이라, 모델이 성공 패턴을 안정적으로 일반화하기엔 한계가 있었다. 이 때문에 ML 점수는 단독 판단이 아닌 보조 지표로 활용했다.
- **초점 전환**: 제한된 데이터 환경을 고려해, 정확한 예측 모델 구축보다 입지 특성 정량화와 추천 구조 설계에 분석의 초점을 두었다.
- **다음 단계**: 유사 저가 커피 프랜차이즈 데이터를 포함해 학습 데이터를 수백~수천 건 규모로 확장하면 일반화 성능을 개선할 수 있을 것으로 보인다.
